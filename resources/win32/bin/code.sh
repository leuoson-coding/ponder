...
if [ "$PONDER_WSL_DEBUG_INFO" = true ]; then
	set -x
fi
...
PONDER_PATH="$(dirname "$(dirname "$(realpath "$0")")")"
ELECTRON="$PONDER_PATH/$NAME.exe"
...
if [ $IN_WSL = true ]; then
	...
	CLI=$(wslpath -m "$PONDER_PATH/resources/app/out/cli.js")
	...
	CLI=$(cygpath -m "$PONDER_PATH/resources/app/out/cli.js")
else
	CLI="$PONDER_PATH/resources/app/out/cli.js"
fi
